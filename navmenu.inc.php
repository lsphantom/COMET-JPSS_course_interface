...
{	
	...
	function Navmenu($server = NULL, $path = NULL, $tab, $type, $linkPrefix = "navmenu.php", $linkSuffix = "")
	{
		$this->server = $server;
		$this->path = $path;
		$this->tab = $tab;
		...
		$this->toc = $this->html->find('div[id=tableofcontents]', 0);
	...
	{
		$content = $this->html->find('div[id=page_' . $page . ']', 0);
	...
	public function getTypeToggleLink($page)
	{
		...
		return "<a class=\"switch\" href=\"" . $this->linkPrefix . "?tab=" . $this->tab . "&amp;page=" . $page . "&amp;type=" . $t . $this->linkSuffix . "\">Switch to <span>". $tt . "</span></a>";
